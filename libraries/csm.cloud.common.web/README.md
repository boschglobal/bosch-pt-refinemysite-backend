[![Build Status](https://dev.azure.com/pt-iot/smartsite/_apis/build/status/csm.cloud.common.web?branchName=master)](https://dev.azure.com/pt-iot/smartsite/_build/latest?definitionId=56?branchName=master)

# Introduction

This repo contains code for common functionality of the smartsite backend services based on spring web (non-reactive).
...
To create a new release first all changes have to be committed and pushed to the remote repo.
Then you can create the release by executing the :rocket: *__Azure DevOps YAML Pipeline__* which allows you to select the version-portion to increment or by using the following command:
