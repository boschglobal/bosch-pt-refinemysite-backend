      ...
      namespace.tables.forEach { table: Table ->
        table.foreignKeys.values.forEach skipLabel@{ foreignKey: ForeignKey ->
          val uniqueKeyColumns =
              Iterable { table.uniqueKeys.values.iterator() }
                  .map { it.columns }
                  .filter { it.size == 1 }
                  .flatten()
          ...
          // Create index name
          val shortColumnName =
              columns.joinToString(separator = "") { column: Column ->
                getShortenedName(column.name)
              }
    ...
    if (inputString.contains("_")) {
      ...
      inputString =
          parts.joinToString(separator = "") {
            it.lowercase(Locale.US).substring(0, Math.min(4, it.length)).capitalizeWords()
          }
    } else {
