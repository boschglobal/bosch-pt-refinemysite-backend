...
import com.bosch.pt.csm.application.config.KafkaTopicProperties
import com.bosch.pt.csm.application.config.KafkaTopicProperties.Companion.COMPANY_BINDING
...
@Component
class CompanyContextEventStore(
    ...
    AbstractEventStore<CompanyContextKafkaEvent>(
        blockModifyingOperations, em, kafkaProperties, environment, schemaRegistryClient) {

  override fun getNameOfEventTopic() = kafkaTopicProperties.getTopicForChannel(COMPANY_BINDING)

  override fun getNumberOfPartitionsOfEventTopic() =
      kafkaTopicProperties.getConfigForChannel(COMPANY_BINDING).partitions
  ...
  override fun createKafkaEvent(
      ...
      eventKey: ByteArray,
      event: ByteArray?
  ): CompanyContextKafkaEvent =
      CompanyContextKafkaEvent(traceHeaderKey, traceHeaderValue, partitionNumber, eventKey, event)
          .also {
