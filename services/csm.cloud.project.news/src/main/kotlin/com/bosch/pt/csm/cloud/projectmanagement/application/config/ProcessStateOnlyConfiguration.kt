...
@Profile("!test")
class ProcessStateOnlyConfiguration(
  ...
  fun logOnInit() {
    if (processStateOnlyProperties.isEnabled) {
      LOGGER.warn(
          "Process state only is enabled. Events created before " +
              "${processStateOnlyProperties.untilDate} will not produce news or events.")
    } else {
