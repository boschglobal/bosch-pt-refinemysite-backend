...
class CustomUserAuthenticationConverterTest {
  ...
  fun verifyConvertUser() {
    ...
    val headers = mapOf("fakeHeader" to randomUUID() as Any)
    val jwt = Jwt(randomUUID().toString(), null, null, headers, claims)
    val user = User(randomUUID().toString(), randomUUID(), false, false)
    ...
    assertThat(authentication.authorities)
        .containsAll(AuthorityUtils.createAuthorityList(USER.roleName()))
  }
  ...
  fun verifyConvertAdmin() {
    ...
    val headers = mapOf("fakeHeader" to randomUUID() as Any)
    val jwt = Jwt(randomUUID().toString(), null, null, headers, claims)
    val user = User(randomUUID().toString(), randomUUID(), true, false)
    ...
    assertThat(authentication.principal).isEqualTo(user)
    assertThat(authentication.authorities)
        .containsAll(AuthorityUtils.createAuthorityList(USER.roleName(), ADMIN.roleName()))
  }
