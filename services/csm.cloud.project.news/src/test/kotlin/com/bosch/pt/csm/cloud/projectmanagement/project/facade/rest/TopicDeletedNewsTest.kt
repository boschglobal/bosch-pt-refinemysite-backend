...
class TopicDeletedNewsTest : AbstractNewsTest() {
  ...
  fun `the news should remain`(
    ...
    eventStreamGenerator.submitTopicG2(eventType = TopicEventEnumAvro.DELETED)

    // Ensure that news are still there
    assertThat(controller.getDetails(employee, taskAggregateIdentifier))
        .matchesNewsInAnyOrder(
            buildNews(taskAggregateIdentifier, taskAggregateIdentifier, taskAggregateIdentifier),
            buildNews(taskSchedule, taskAggregateIdentifier, taskAggregateIdentifier),
  ...
  fun `object relations should be gone`(
    ...
    val topicObjectIdentifier = ObjectIdentifier(topic)
    val taskRelation =
        objectRelationRepository.findOneByLeftAndRightType(topicObjectIdentifier, TASK)
    assertThat(taskRelation).isNotNull
    val taskObjectIdentifier = requireNotNull(taskRelation!!.right)
    ...
    assertThat(
            objectRelationRepository.findAllByLeftTypeAndRightIn(
                TOPIC_ATTACHMENT, Collections.singletonList(topicObjectIdentifier)))
        .isNotEmpty

    val messageRelations =
        objectRelationRepository.findAllByLeftTypeAndRightIn(
            MESSAGE, Collections.singletonList(topicObjectIdentifier))
    ...
    // Ensure that task related object relations remain
    assertThat(
            objectRelationRepository.findAllByLeftTypeAndRight(
                TASK_ATTACHMENT, taskObjectIdentifier))
        .isNotEmpty
    ...
    // Ensure that topic related relations are deleted
    assertThat(objectRelationRepository.findOneByLeftAndRightType(topicObjectIdentifier, TASK))
        .isNull()
