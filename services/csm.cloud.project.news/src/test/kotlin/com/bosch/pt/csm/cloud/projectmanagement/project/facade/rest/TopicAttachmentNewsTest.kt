...
class TopicAttachmentNewsTest : AbstractNewsTest() {

  private lateinit var topicAggregateIdentifier: AggregateIdentifierAvro
  private lateinit var topicAttachmentAggregateIdentifier: AggregateIdentifierAvro
  ...
  fun initialize() {
    ...
    eventStreamGenerator.submitTopicAttachment()
    topicAttachmentAggregateIdentifier = getByReference("topicAttachment")
  }
  ...
  fun details(@Suppress("UNUSED_PARAMETER") role: String, employee: () -> EmployeeAggregateAvro) {
    assertThat(controller.getDetails(employee, taskAggregateIdentifier))
        .matchesNewsInAnyOrder(
            ...
            buildNews(
                topicAttachmentAggregateIdentifier,
                topicAggregateIdentifier,
                taskAggregateIdentifier))
  }

  @MethodSource(employeesCsmCrFm)
  @ParameterizedTest(name = "for {0}")
  fun `is created after creation of a topic attachment`(
      @Suppress("UNUSED_PARAMETER") role: String,
      employee: () -> EmployeeAggregateAvro
  ) {
