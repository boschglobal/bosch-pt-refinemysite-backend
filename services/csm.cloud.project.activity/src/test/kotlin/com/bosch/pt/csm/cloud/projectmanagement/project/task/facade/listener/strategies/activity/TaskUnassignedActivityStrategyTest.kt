...
class TaskUnassignedActivityStrategyTest : AbstractActivityIntegrationTest() {

  private val task by lazy { context["task"] as TaskAggregateAvro }

  @Test
  fun `when a task is unassigned`() {
    eventStreamGenerator
        .setUserContext("cr-user")
        .submitTask { it.assignee = getByReference("fm-participant") }
        .submitTask(eventType = UNASSIGNED) { task ->
          task.assignee = null
          task.status = DRAFT
        }
