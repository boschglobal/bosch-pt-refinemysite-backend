...
@DisplayName("Verify topic escalated activity")
@SmartSiteSpringBootTest
class TopicEscalatedActivityStrategyTest : AbstractActivityIntegrationTest() {
  ...
  fun `when the topic has description`() {
    eventStreamGenerator.submitTopicG2(eventType = ESCALATED) { it.description = "description" }
    ...
    requestActivities(task)
        ...
        .andExpect(hasSummary(buildSummary(TOPIC_ACTIVITY_ESCALATED, true)))
  }

  @Test
  fun `when the topic does not have description`() {
    eventStreamGenerator.submitTopicG2(eventType = ESCALATED) { it.description = null }
    ...
    requestActivities(task)
        ...
        .andExpect(hasDate(timeLineGenerator.time))
        .andExpect(hasUser(fmUser))
        .andExpect(hasSummary(buildSummary(TOPIC_ACTIVITY_ESCALATED_WITHOUT_TEXT, false)))
  }
