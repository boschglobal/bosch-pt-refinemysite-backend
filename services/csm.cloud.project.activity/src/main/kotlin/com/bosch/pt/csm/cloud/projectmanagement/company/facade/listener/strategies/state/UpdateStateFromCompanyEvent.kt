...
import com.bosch.pt.csm.cloud.projectmanagement.company.facade.listener.message.toEntity
...
class UpdateStateFromCompanyEvent(private val companyService: CompanyService) :
  ...
  override fun updateState(key: EventMessageKey, event: CompanyEventAvro) =
      event.getAggregate().run {
        companyService.save(toEntity())
        companyService.deleteByVersion(getIdentifier(), getAggregateIdentifier().getVersion() - 1)
      }
}
