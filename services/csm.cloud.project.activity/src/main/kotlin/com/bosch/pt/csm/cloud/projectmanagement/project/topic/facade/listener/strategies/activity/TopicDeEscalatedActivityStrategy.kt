...
@Component
class TopicDeEscalatedActivityStrategy(
  ...
  override fun handles(key: EventMessageKey, value: SpecificRecordBase?): Boolean =
      value is TopicEventG2Avro && value.getName() == DEESCALATED
  ...
  private fun buildSummary(
    ...
    return when (topicAggregateAvro.getDescription()) {
      null ->
          Summary(
              templateMessageKey = TOPIC_ACTIVITY_DEESCALATED_WITHOUT_TEXT,
              references = mapOf("originator" to originator))
      else ->
          Summary(
              templateMessageKey = TOPIC_ACTIVITY_DEESCALATED,
              references =
                  mapOf(
