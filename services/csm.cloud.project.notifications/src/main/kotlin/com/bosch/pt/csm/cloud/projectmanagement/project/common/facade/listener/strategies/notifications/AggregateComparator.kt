...
class AggregateComparator(private val mongoOperations: MongoOperations) {
  fun <T> compare(currentVersion: T?, previousVersion: T?): MutableMap<String, Any?> {
    ...
    val changedAttributes: MutableMap<String, Any?> =
        differences
            .entriesDiffering()
            // attributes with a changed value
            .map { entry -> Pair(entry.key, entry.value.rightValue()) }
            .toMap()
            // removed attributes (value is mapped to null to not have the old value in the list of
            // changes)
            .plus(differences.entriesOnlyOnLeft().map { entry -> Pair(entry.key, null) }.toMap())
            // added attributes (no mapping required, since it only contains a single value being
            // the new one)
            .plus(differences.entriesOnlyOnRight())
            .toMutableMap()

    // the version always differs, therefore, it is removed here
    changedAttributes.remove("_id.version")
  ...
  private fun toMap(document: Document, keyPrefix: String): Map<String, Any?> {
    ...
    for (entry in document.entries) {
      ...
      if (value is Document) {
        map.putAll(toMap(value, prefixedKey))
      } else if (prefixedKey != "_id.version") {
        map[prefixedKey] = value
      }
    }
    return map
  }
