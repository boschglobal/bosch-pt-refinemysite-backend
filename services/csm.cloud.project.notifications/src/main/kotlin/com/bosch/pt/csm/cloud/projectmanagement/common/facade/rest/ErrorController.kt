...
import com.bosch.pt.csm.cloud.common.exceptions.CommonErrorUtil
...
class ErrorController(
  ...
  fun handleMethodArgumentValidationExceptions(
    ...
    LOGGER.info(
        "Request contained invalid arguments in following fields: " +
            "${ex.bindingResult.fieldErrors.map { it.field }.toList()}")
    MDC.remove(RESPONSE_HTTP_STATUS_CODE)
    return buildErrorResponse(translatedErrorMessage, BAD_REQUEST)
  }
  ...
  private fun buildErrorResponse(
    ...
    return ResponseEntity(
        ErrorResource(message.translatedMessage, GlobalTracer.get().traceId), status)
  }
  ...
  private fun getTranslatedErrorMessage(
    ...
    return CommonErrorUtil.getTranslatedErrorMessage(
        messageSource, messageKey ?: alternateMessageKey!!, locale, rootCause)
  }

  private fun logError(@NonNull message: String, @NonNull throwable: Throwable) {
    if (environment.acceptsProfiles(Profiles.of(PROFILE_LOG_JSON))) {
  ...
  private fun logWarning(@NonNull message: String, @NonNull throwable: Throwable) {
    if (environment.acceptsProfiles(Profiles.of(PROFILE_LOG_JSON))) {
  ...
  companion object {
    ...
    private const val PROFILE_LOG_JSON = "log-json"
