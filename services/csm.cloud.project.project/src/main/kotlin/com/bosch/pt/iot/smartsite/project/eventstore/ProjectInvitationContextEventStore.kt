...
@Component
open class ProjectInvitationContextEventStore(
    ...
    AbstractEventStore<ProjectInvitationContextKafkaEvent>(
        blockModifyingOperations, em, kafkaProperties, environment, schemaRegistryClient) {

  override fun getNameOfEventTopic() =
      kafkaTopicProperties.getTopicForChannel(PROJECT_INVITATION_BINDING)

  override fun getNumberOfPartitionsOfEventTopic() =
      kafkaTopicProperties.getConfigForChannel(PROJECT_INVITATION_BINDING).partitions
  ...
  override fun createKafkaEvent(
      ...
      eventKey: ByteArray,
      event: ByteArray?
  ): ProjectInvitationContextKafkaEvent =
      ProjectInvitationContextKafkaEvent(
              eventKey,
              event,
              partitionNumber,
