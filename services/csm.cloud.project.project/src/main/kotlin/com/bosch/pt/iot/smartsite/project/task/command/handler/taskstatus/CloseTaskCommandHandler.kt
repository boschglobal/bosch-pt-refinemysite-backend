...
@Component
open class CloseTaskCommandHandler(
  ...
  open fun handle(@AuthorizationCacheKey identifier: TaskId) {
    snapshotStore
        ...
        .toCommandHandler()
        .checkPrecondition { isCloseTaskPossible(it.status) }
        .onFailureThrow(TASK_VALIDATION_ERROR_CLOSED_OR_ACCEPTED_TASK_CLOSE_FORBIDDEN)
        .update {
          LOGGER.info(TASK_STATUS_CHANGE_MSG, it, it.status, CLOSED)
          it.copy(status = CLOSED)
        }
        .emitEvent(TaskEventEnumAvro.CLOSED)
  ...
  companion object {
    private val LOGGER = getLogger(CloseTaskCommandHandler::class.java)
