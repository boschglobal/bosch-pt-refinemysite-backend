...
import com.bosch.pt.csm.cloud.common.translation.CommonStreamableKey.COMMON_VALIDATION_ERROR_ENTITY_OUTDATED
...
internal class ErrorControllerTest {
  ...
  fun verifyHandleMethodArgumentValidationExceptions() {
    ...
    val mockAppender: Appender<ILoggingEvent> = mockk()
    val logger = LoggerFactory.getLogger(ROOT_LOGGER_NAME) as Logger
    logger.level = INFO

    val loggingEvent = slot<LoggingEvent>()
    every { mockAppender.doAppend(capture(loggingEvent)) } returns Unit
    ...
    assertThat(loggingEvent.captured.level).isEqualTo(INFO)
    assertThat(loggingEvent.captured.formattedMessage)
        .isEqualTo("Request contained invalid arguments in following fields: [field1, field2]")
  }
  ...
  @Test
  @Disabled
  fun // Mocking or constructing a proper MethodArgumentNotValidException is quite tricky
  verifyBadRequestWithMethodArgumentNotValidException() {
  ...
  fun verifyHandleEntityOutdatedException() {
    val exception =
        com.bosch.pt.csm.cloud.common.command.exceptions.EntityOutdatedException(
            COMMON_VALIDATION_ERROR_ENTITY_OUTDATED)
    val responseEntity = cut.handleOptimisticLockingFailureException(exception)
