...
class CloseTaskCommandHandlerAuthorizationTest : AbstractTaskServiceAuthorizationIntegrationTest() {
  ...
  fun `verify close a task that CR is assigned to company and FM is assigned to task`(
    ...
    checkAccessWith(userType) { closeTask(taskAssigned) }
  }

  @ParameterizedTest
  @MethodSource("csmAndOtherCompanyWithAccess")
  fun `verify close a task that CR is not assigned to company and FM is not assigned to task`(
      userType: UserTypeAccess
  ) {
    taskAssigned.status = OPEN
    doWithAuthorization(userMap[CSM]!!) {
  ...
  @ParameterizedTest
  @MethodSource("csmAndCrAndAssignedFmWithAccess")
  fun `verify close open task is authorized for`(userType: UserTypeAccess) {
    doWithAuthorization(userMap[CSM]!!) {
      assignTask(taskAssigned, taskAssigned.assignee!!)
      sendTask(taskAssigned)
    }
