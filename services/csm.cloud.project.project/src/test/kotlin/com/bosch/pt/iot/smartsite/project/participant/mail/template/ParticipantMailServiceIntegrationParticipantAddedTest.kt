...
import com.bosch.pt.iot.smartsite.project.participant.mail.template.ParticipantAddedTemplate.Companion.ORIGINATOR_NAME
import com.bosch.pt.iot.smartsite.project.participant.mail.template.ParticipantAddedTemplate.Companion.PROJECT_NAME
import com.bosch.pt.iot.smartsite.project.participant.mail.template.ParticipantAddedTemplate.Companion.PROJECT_URL
import com.bosch.pt.iot.smartsite.project.participant.mail.template.ParticipantAddedTemplate.Companion.RECIPIENT_NAME
import com.bosch.pt.iot.smartsite.project.participant.mail.template.ParticipantAddedTemplate.Companion.TEMPLATE_NAME
...
@DisplayName("Verify participant invitation mail A (added to project")
@EnableAllKafkaListeners
open class ParticipantMailServiceIntegrationParticipantAddedTest : AbstractIntegrationTestV2() {
  ...
  private val templateInvitationProperties by lazy { mailProperties.templates[TEMPLATE_NAME]!! }
  ...
  fun `is sent to invited participant`() {
    transactionTemplate.execute {
      cut.sendParticipantAdded(project.identifier, invitedParticipant, invitingParticipant)
    }
    ...
    assertThat(recipient.getString("Name")).isEqualTo(invitedParticipant.getDisplayName())
  }
  ...
  fun `uses the correct template id for country code`() =
      templateInvitationProperties.countries.map {
  ...
  fun `uses default template for unsupported countries`() {
    val defaultTemplateId = templateInvitationProperties.default
  ...
  fun `uses template based on country of street address if given`() {
    val expectedTemplateId = templateInvitationProperties.countries["DE"]!!
  ...
  fun `uses template based on country of post box address if no street address is given`() {
    val expectedTemplateId = templateInvitationProperties.countries["US"]!!
    eventStreamGenerator.submitCompanyWithPostboxAddress {
      it.postBoxAddressBuilder.country = "United States of America (the)"
    }
