...
class DayCardApprovalIntegrationTest : AbstractIntegrationTestV2() {
  ...
  @Nested
  inner class `changing the state to approved` {

    @Nested
    inner class `is successful` {

      @Test
      fun `from 'OPEN' to 'APPROVED'`() {
        val response = cut.approveDayCard(dayCard1task1, ETag.from(0))

        assertThat(response.body!!.status).isEqualTo(DayCardStatusEnum.APPROVED)
        projectEventStoreUtils.verifyContains(
            DayCardEventG2Avro::class.java, DayCardEventEnumAvro.APPROVED, 1, true)
      }

      @Test
      fun `for multiple day cards`() {
        val response =
            cut.approveMultipleDayCards(
        ...
        assertThat(response.body!!.items.map { it.status }).containsOnly(DayCardStatusEnum.APPROVED)
        projectEventStoreUtils.verifyContains(
            DayCardEventG2Avro::class.java, DayCardEventEnumAvro.APPROVED, 2, false)
      }

      @Test
      fun `for multiple day cards from different tasks are approved at once`() {
        val response =
            cut.approveMultipleDayCards(
                VersionedUpdateBatchRequestResource(
                    setOf(
    ...
    inner class `reports error` {
      ...
      @Test
      fun `if day card is approved that does not exist`() {
        val response =
            catchThrowableOfType(
                { cut.approveDayCard(DayCardId(), ETag.from(0)) },
      ...
      @Test
      fun `if day card is approved that is already in status 'APPROVED'`() {
        val response =
            catchThrowableOfType(
                { cut.approveDayCard(dayCard2task1, ETag.from(0)) },
      ...
      @Test
      fun `if day card is approved that is in non-'OPEN' state`() {
        val response =
            catchThrowableOfType(
                { cut.approveDayCard(dayCard4task1, ETag.from(0)) },
                PreconditionViolationException::class.java)
      ...
      @Test
      fun `if multiple day cards are approved at once where one does not exist`() {
        val response =
            catchThrowableOfType(
                {
                  cut.approveMultipleDayCards(
      ...
      @Test
      fun `if multiple day cards are approved at once where one has a wrong status`() {
        val response =
            catchThrowableOfType(
                {
                  cut.approveMultipleDayCards(
