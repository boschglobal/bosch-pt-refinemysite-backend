...
import org.apache.commons.text.WordUtils.capitalize
    ...
    metadata.database.namespaces.forEach { namespace ->
      namespace.tables.forEach { table: Table ->
        table.foreignKeys.values.forEach foreignKeysExitLabel@{ foreignKey: ForeignKey ->
          val iterable = Iterable { table.uniqueKeys.values.iterator() }
    ...
    if (inputString.contains("_")) {
      ...
      inputString =
          parts.joinToString(separator = "") {
            capitalize(it.lowercase(Locale.US).substring(0, Math.min(4, it.length)))
          }
    } else {
