...
class BasicPatHttpConfigurer<H : HttpSecurityBuilder<H>>(context: ApplicationContext) :
    AbstractPatHttpConfigurer<BasicPatHttpConfigurer<H>, H>(context) {

  init {
    val patResolver =
        if (context.getBeanNamesForType(BasicPatResolver::class.java).isNotEmpty()) {
          context.getBean(BasicPatResolver::class.java)
        } else {
          BasicPatResolver()
        }
  ...
  override fun configure(http: H) {
    ...
    tokenResolver.setAuthenticationConverter(basicAuthenticationConverter)
...
fun HttpSecurity.basicAuthentication(): BasicPatHttpConfigurer<out HttpSecurityBuilder<*>> {
  val applicationContext = this.getSharedObject(ApplicationContext::class.java)
  return this.apply(BasicPatHttpConfigurer(applicationContext)).also { it.postProcess() }
}
