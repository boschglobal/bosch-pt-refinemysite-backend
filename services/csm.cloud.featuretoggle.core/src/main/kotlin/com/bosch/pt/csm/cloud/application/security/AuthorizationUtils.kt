...
object AuthorizationUtils {
  ...
  fun <R : Collection<UUID>?, U : Collection<UUID>> checkAuthorizationForMultipleResults(
    ...
    return !target.isNullOrEmpty() && target.containsAll(specifier)
  }
   ...
   */
  fun hasRoleAdmin(): Boolean = SecurityContextHelper.hasRole(RoleConstants.ADMIN.name)
  ...
  fun setUserAuthentication(user: UserProjection) {
    val roles: MutableCollection<SimpleGrantedAuthority> =
        mutableListOf(SimpleGrantedAuthority(RoleConstants.USER.roleName()))
    if (user.admin) {
      roles.add(SimpleGrantedAuthority(RoleConstants.ADMIN.roleName()))
    }
